...
```
All figure handles are closed before returning to free RAM.
"""
# Use only the last 60 candles — enough context, minimal RAM
...
“””
Return USDT spot pairs whose last price is under MAX_PRICE,
sorted by 24 h quote volume (most liquid first), capped at MAX_PAIRS.
“””
try:
...
# Sort by volume descending, take top MAX_PAIRS
...
# Track alerted pairs within the current cycle to avoid duplicate alerts
alerted_this_cycle: set[str] = set()

while True:
    cycle_start = time.time()

    # Reset alert deduplication every N cycles (~30 min)
    ...
    for symbol in pairs:
        ...
        # Brief pause between pairs to respect rate limits
        time.sleep(0.5)
